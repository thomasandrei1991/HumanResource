<?php
    ...
    $checkQuery = mysqli_prepare($conn, "SELECT id, time_in, time_out
        FROM attendance
        WHERE employee_id = ?
    ...
    if (mysqli_num_rows($result) === 0) {
        header("Location: employee_dashboard.php?error=no_time_in");
        exit();
    }
    ...
    if (!empty($attendance['time_out'])) {
        header("Location: employee_dashboard.php?error=already_out");
        exit();
    }
