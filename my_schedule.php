<?php
    ...
    $userStmt = mysqli_prepare($conn, "SELECT employee_id
        FROM users
        WHERE id = ?
        LIMIT 1"
    );
    ...
    if ($employeeId) {
        $employeeStmt = mysqli_prepare($conn, "SELECT id, employee_id, firstname, lastname, department, position
            FROM employees
            WHERE id = ?
            LIMIT 1"
        );
    ...
    if ($employeeId) {
        ...
        $scheduleStmt = mysqli_prepare($conn, "SELECT id, schedule_date, time_in, time_out, break_start, break_end, created_at
            FROM schedules
            WHERE employee_id = ?
...
<head>
    ...
    <link rel="stylesheet" href="styles/my_schedule.css">
                ...
                <?php if ($employee): ?>
                    <div class="employee-info-card">
                        <div class="employee-avatar">
                            <?php $initials = strtoupper(substr($employee['firstname'], 0, 1) .substr($employee['lastname'], 0, 1));
                        ...
                        <div class="employee-details">
                            ...
                            <p>Employee ID:
                                <?php 
                                    echo htmlspecialchars($employee['employee_id']);
                                ?>
                            </p>
                            <p>Department:
                                <?php
